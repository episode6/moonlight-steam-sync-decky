...
describe("cliStatus (spec 3.6.1)", () => {
  it("missing", () => {
    expect(cliStatus({ installed: null, minimum: "0.3.0", too_old: false })).toEqual({
      state: "missing",
  ...
  it("too old", () => {
    expect(cliStatus({ installed: "0.2.0", minimum: "0.3.0", too_old: true })).toEqual({
      state: "too-old",
      text: "CLI too old (0.2.0, needs 0.3.0) — see About",
    });
    // the frontend agrees even if the flag were missing
    expect(cliStatus({ installed: "0.2.9", minimum: "0.3.0", too_old: false }).state).toBe("too-old");
  });

  it("ok for the minimum and newer", () => {
    expect(cliStatus({ installed: "0.3.0", minimum: "0.3.0", too_old: false }).state).toBe("ok");
    expect(cliStatus({ installed: "1.0.0", minimum: "0.3.0", too_old: false }).text).toBe("");
  });

  it("explains a zip built before the CLI release", () => {
    expect(bundledLine({ bundled: null, minimum: "0.3.0" })).toBe(
      "none (built before the CLI release); install it with the CLI's install.sh",
    );
    expect(bundledLine({ bundled: "0.3.0", minimum: "0.3.0" })).toBe("0.3.0");
  });
});
