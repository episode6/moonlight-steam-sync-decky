...
def test_search_and_pin_results_never_carry_the_key(make_backend, tmp_path) -> None:
    ...
    (fixture / "match.ndjson").write_text(
        start % "match"
        + json.dumps({"event": "note", "message": f"key {KEY}"})
        + "\n"
        + '{"event":"pinned","name":"Hades II","steam_appid":1145350,"sgdb_id":null,'
        '"override_line":null}\n'
    )
    backend = make_backend(env={"FAKE_CLI_FIXTURES": str(fixture), "SGDB_API_KEY": KEY})
    for result in (run(backend.search("Hades II")), run(backend.pin("Hades II", 1145350))):
        assert result["ok"] is True
